...
urls = ["https://google.com", "http://nytimes.com", "http://cnn.com"]

i = 0
regex = (r'<title>(.+?)</title>')   # gets whatever text is between the title tags, means non-greedy match one or more chars
...
while i < len(urls):
    ...
    print(htmltext[:500])
    titles = re.findall(pattern, htmltext)	# each title for each chunk of htmltext as a string
    print('\n\n')
    print(titles)
    print("---***---***---***---***---***---***---***---***---")   # url separator
    i += 1	
